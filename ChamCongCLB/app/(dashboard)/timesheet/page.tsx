...
export default function ClassPage() {
    ...
    },
    {
      ...
      render: (studentsId: number[]) => {
        const studentNames = studentsId
          ...
          .join(', ')
        return studentNames.length > 20
          ? `${studentNames.slice(0, 20)}...`
          : studentNames
      },
    },
    {
  ...
  const steps = [
    {
      title: 'Thềm chấm công',
      content: 'Ấn vào nút này để thêm chấm công mới.',
      targetId: 'btn-add',
    },
    {
      title: 'Tìm kiếm',
      content: 'Nhập từ khóa vào ô tìm kiếm để tìm kiếm.',
      targetId: 'search-input',
    },
    {
      title: 'Tìm kiếm',
      content: 'Ấn để tìm kiếm',
      targetId: 'btn-search',
    },
    {
      title: 'Đặt lại',
      content: 'Ấn để đặt lại tìm kiếm',
      targetId: 'btn-reset',
    },
  ]
  ...
  return (
    <>
      ...
      {isLoading || classes.length === 0 || students.length === 0 ? (
        <SpinLoading />
      ) : (
        <DataGrid<ITimesheet>
          rowKey="id"
      ...
      <CustomModal
        ...
        footer={
          <Space>
            <Button
              type="primary"
              onClick={() => selectedTimesheet && handleUpdate()}
            >
              Cập nhật
            </Button>
            <Button type="default" onClick={handleClose}>
